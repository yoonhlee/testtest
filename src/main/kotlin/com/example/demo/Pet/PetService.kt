...
class PetService(
     ...
     */
    @Transactional
    fun createPet(userId: Long, request: PetCreateRequest): PetResponse {
        // 1. 주인을 찾는다 (없으면 예외 발생)
    ...
    fun getPetsByUserId(userId: Long): List<PetResponse> {
        ...
        return pets.map { pet -> PetResponse.from(pet) }
    }
     ...
     */
    @Transactional
    fun updatePet(userId: Long, petId: Long, request: PetUpdateRequest): PetResponse {
